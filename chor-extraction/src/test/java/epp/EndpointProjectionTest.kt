...
class EndpointProjectionTest : Assert() {

    private var epp: EndPointProjection? = null

    @org.junit.Before
    @Throws(Exception::class)
    fun setUp() {
        epp = EndPointProjection()
    }

    @Theory
    @Throws(MergingException::class)
    fun testProjection(vararg testData: Any) {
        val test = testData[0] as String
        println("\n" + "Choreography: " + test)

        val actual = epp!!.project(test).toString()
        println("Network: " + actual)
        Assert.assertEquals(test, testData[1], actual)
