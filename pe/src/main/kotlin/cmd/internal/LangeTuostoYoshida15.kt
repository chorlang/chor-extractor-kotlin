...
object LangeTuostoYoshida15 {
    fun runningExample() = TestUtils.runExtractionTest(
    ...
    fun runningExample2x() = TestUtils.runExtractionTest(
    ...
    fun health() = TestUtils.runExtractionTest(
    ...
    fun health2x() = TestUtils.runExtractionTest(
                "hs{def X{p?; ss!<subscribed>; ss&{" +
                        "ok: p+subscribed; as!<account>; as?; t!<fwd>; t?; X, " +
                        "nok: p+notSubscribed; X}} main{X}} | " +
                        "p{def X{hs!<sendData>; hs&{subscribed: es?; X, notSubscribed: X}} main{X}} | " +
    ...
    fun filter() = TestUtils.runExtractionTest(
                "filter {" +
    ...
    fun filter2x() = TestUtils.runExtractionTest(
                "filter1 {" +
    ...
    fun logistic() = TestUtils.runExtractionTest(
                "supplier {" +
                        "def X {shipper?; Y} " +
                        "def Y {if needToShip " +
                        "then shipper+item; X " +
    ...
    fun logistic2x() = TestUtils.runExtractionTest(
                "supplier {" +
                        "def X {shipper?; Y} " +
                        "def Y {if needToShip " +
                        "then shipper+item; X " +
    ...
    fun logistic2() = TestUtils.runExtractionTest(
                "supplier {" +
    ...
    fun cloudSystem() = TestUtils.runExtractionTest(
                "cl{" +
    ...
    fun cloudSystem2x() = TestUtils.runExtractionTest(
                "cl{" +
                        "def X{int!<connect>; int?; Y} " +
                        "def Y{if access then appli+awaitcl; appli!<access>; Y else int!<logout>; appli+syncLogout; appli?; X} " +
                        "main {X}} | " +
    ...
    fun sanitaryAgency() = TestUtils.runExtractionTest(
                "citizen{" +
                        "def X{" +
    ...
    fun sanitaryAgency2x() = TestUtils.runExtractionTest(
                "citizen{" +
                        "def X{" +
                        "sanagency!<request>; sanagency?; sanagency!<provInf>; sanagency&{" +
                        "refusal: X, " +
