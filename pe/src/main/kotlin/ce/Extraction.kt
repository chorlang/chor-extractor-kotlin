...
class NetworkExtraction {
    ...
    private fun buildGraph(bldgrphnode: ConcreteNode, graph: DirectedGraph<ConcreteNode, ExtractionLabel>): Boolean {
        ...
        //sort processes by the strategy passed from the outside
        //TODO("strategy as a param")
        ...
        for (p in nodesorted) {
            ...
            if (findComm != null) {
                ...
                unfolded.forEach { tgt1.network[it]?.main = bldgrphnode.nodenet.network[it]?.main?.copy()!! }
                unfolded.forEach { tgt2.network[it]?.main = bldgrphnode.nodenet.network[it]?.main?.copy()!! }
        ...
        for (p in nodesorted) {
            ...
            val b = getInteractionLabel(p.key, nodesorted)
    ...
    private fun sortProcesses(node: ConcreteNode): HashMap<String, ProcessBehaviour> {
        val net = node.nodenet.network
        val copynet = LinkedHashMap<String, ProcessBehaviour>()

        //put Selection/Offering on top
    ...
    fun unfold(p: String, n: network): Boolean {
        ...
        return if (pbmain is ProcedureInvocationSP && (!pbmain.visited|| isAllProceduresVisited(n)) ) {

            val pname = pbmain.procedure
            val pdef = pb.procedures[pname]

            pb.main = pdef?.behaviour?.copy() ?: throw Exception("Can't unfold the process")
    ...
    private fun isProcedureVisited(bh: Behaviour): Boolean {
        return when (bh) {
            is Sending -> isProcedureVisited(bh.continuation)
            is Receiving -> isProcedureVisited(bh.continuation)
            is SelectionSP -> isProcedureVisited(bh.continuation)
            is Offering -> bh.labels.all{isProcedureVisited(it.component2())}
