...
class UnfolderTest {
    companion object {
        private fun unfold(orig:String) {
            val unfolded1 = NetworkUnfolder.compute(orig, 0.2, 3)
            val unfolded2 = NetworkUnfolder.compute(orig, 0.4, 2)
            val unfolded3 = NetworkUnfolder.compute(orig, 0.7, 1)
            println("Original: $orig\nUnfolded(0.2,3): $unfolded1\nUnfolded(0.4,2): $unfolded2\nUnfolded(0.7,1):$unfolded3")
        }
    }
